...
from cursor_position_monitor import get_sentences, get_line_range
...
if __name__ == "__main__":
    ...
    print("CustomBlock 테스트 (Enter로 다음, q로 종료)")
    print("-" * 40)
    ...
    print(f"현재 위치: 문단={para_id}, pos={char_pos}\n")

    # 디버깅: pos_map 출력
    pos_list, para_text, para_end_pos = block._get_pos_map(para_id)
    print(f"문단 텍스트 (len={len(para_text)}): '{para_text[:50]}...'")
    print(f"pos_list (처음 20개): {pos_list[:20]}")
    print(f"pos_list 길이: {len(pos_list)}, 문단 끝 pos: {para_end_pos}\n")

    # 디버깅: 문장 경계 출력
    sentences, _ = block._get_sentences(para_id)
    print("문장 경계 (HWP pos 기준):")
    for s in sentences:
        print(f"  문장 {s['index']}: pos {s['start']}~{s['end']}")
    print()

    # 디버깅: 줄 경계 출력
    line_starts, para_end = block._get_line_starts(para_id)
    print(f"줄 시작 pos: {line_starts}, 문단 끝: {para_end}")
    print(f"텍스트 길이: {len(para_text)}, HWP 문단 끝 pos: {para_end}")
    print(f"=> pos와 텍스트 인덱스 비율: {para_end / len(para_text) if para_text else 0:.2f}\n")

    tests = [
        ("1. 문단 전체 선택", lambda: block.select_para(para_id)),
        ("2. 현재 pos가 속한 줄 선택", lambda: block.select_line_by_pos(para_id, char_pos)),
        ("3. 첫번째 줄 선택", lambda: block.select_line_by_index(para_id, 0)),
        ("4. 0~1번째 줄 선택", lambda: block.select_lines_range(para_id, 0, 1)),
        ("5. 1~5번째 줄 선택 (없으면 끝까지)", lambda: block.select_lines_range(para_id, 1, 5)),
        ("6. 첫번째 문장 선택", lambda: block.select_sentence(para_id, 1)),
        ("7. 2~4번째 문장 선택", lambda: block.select_sentences_range(para_id, 2, 4)),
        ("8. 2~10번째 문장 선택 (없으면 끝까지)", lambda: block.select_sentences_range(para_id, 2, 10)),
        ("9. 현재 줄 내 문장만 선택", lambda: block.select_sentence_in_line(para_id, char_pos)),
    ]
    ...
    for name, func in tests:
        ...
        if result:
            ...
            print(f"   -> 선택됨: '{text[:30]}{'...' if len(text) > 30 else ''}'")
        else:
            print(f"   -> 실패")

        user_input = input("   [Enter] 다음 / [q] 종료: ")
