...
class TextAlignPage:
    ...
    def fit_spanning_paragraph(self, para_id: int, min_font_size: int = 8) -> Dict:
        """
        페이지 걸친 문단의 글자 크기를 줄여서 이전 페이지로 맞춤

        Args:
            para_id: 문단 ID
            min_font_size: 최소 글자 크기 (pt 단위, 기본 8pt)
    ...
    def fit_all_spanning_paragraphs(self, page: int = None, min_font_size: int = 8) -> Dict:
        """
        특정 페이지 또는 전체 문서에서 페이지 걸친 문단 처리

        Args:
            page: 대상 페이지 (None이면 전체)
            min_font_size: 최소 글자 크기
